...
I have spent nearly three decades building shared abstractions and helping organizations adopt them. NumPy gave scientific Python a common array; SciPy built computational science on top of it; Numba brought compiled speed to the same abstraction; conda made the whole stack distributable; Anaconda, Quansight, and OpenTeams helped enterprises put it to work. None of that was built to be owned by one company. It was built to be shared — and the value accrued broadly, to the communities and companies that adopted it. That was the point, and it is a fair argument that those shared abstractions laid the foundation the AI revolution was built on.

I have also watched what happens when a foundational layer is open and when it is not. When it is open, an ecosystem forms: many companies build, many thrive, standards outlive any single vendor. When it is not, a landlord forms, and everyone else rents. Enterprise AI is at exactly that fork today. Frontier models are magnificent and mostly rented; the context, judgment, and evidence that make them valuable inside an organization are leaking into systems that organization does not control.

This paper is my argument for making the AI era go the first way — and my description of the architecture I believe gets us there: organizations that own their Intelligence Hubs, and a small set of shared abstractions — Frames, Cogs, Ops, Guards, Gates, and Tracks — that many parties can build to, open and commercial. It is not a product pitch. OpenTeams, which I lead, participates in this ecosystem as one contributor among many, and I describe its choices only where they illustrate the pattern. If the argument is right, most of the value it describes will accrue to organizations and builders I have never met. That is what a shared abstraction is for.
...
* **Cold start.** A marketplace with nothing in it attracts no one. The architecture’s answer is that the first artifacts are the ones organizations build for themselves anyway — their own Frames, their own Ops — and that a Hub is valuable before it exchanges anything. Exchange is a second-order benefit, not the entry price.
...
| :---- |
| A distributed AI economy is the Linux \+ App Store for accountable enterprise AI — |
| where Intelligence Hubs are the owned enterprise deployments, |
| open source (Nebari among it) anchors the infrastructure assembled into each Hub, |
...
| and products around the Hub make it all usable by the human at work — built by many, OpenTeams among them. |
...
34\.  IBM. "What is the AI stack?" IBM Think, 2025\. [https://www.ibm.com/think/topics/ai-stack](https://www.ibm.com/think/topics/ai-stack)
